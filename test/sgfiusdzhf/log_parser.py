...
def get_data_from_file(file_path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    while True:
        ...
        if msg_type == "IMU":
            ...
            imu_list.append({"TimeUS": timeUS, "AccX": accX, "AccY": accY, "AccZ": accZ, "GyrX": gyrX, "GyrY": gyrY, "GyrZ": gyrZ})
        elif msg_type == "GPS":
            # Status shows if the drone has connection to GPS. If it does not, Status should be less than 3
            if getattr(msg, "Status", 0) < 3: continue
            timeUS = msg.TimeUS - time_start
            lat = msg.Lat
            lon = msg.Lng
            alt = msg.Alt
            gps_list.append({"TimeUS": timeUS, "Lat": lat, "Lon": lon, "Alt": alt})
